...
export default function TabEvaluacion() {
    ...
    const metricas = useMemo(() => {
        if (!resumenEvaluacion || !Array.isArray(resumenEvaluacion)) return { aptos: 0, cv: 0, ent: 0, porcCV: 0, porcEnt: 0, inscritos: 0 };
        let a = 0, c = 0, e = 0, i = 0;
        resumenEvaluacion.forEach(item => {
            a += Number(item.aptos || 0);
            c += Number(item.evaluados_cv || 0);
            e += Number(item.evaluados_entrevista || 0);
            i += Number(item.inscritos || 0);
        });
        return { 
            aptos: a, 
            cv: c, 
            ...
            porcEnt: i > 0 ? Math.round((e / i) * 100) : 0
        };
    }, [resumenEvaluacion]);
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 shrink-0">
                ...
                <Card className="p-3 xl:p-5 border-l-4 border-green-500 shadow-none border bg-white flex flex-row items-center justify-between h-[75px] xl:h-[110px] 2xl:h-[130px]">
                    <div className="flex flex-col gap-1 xl:gap-2">
                        ...
                        <h3 className="text-xl xl:text-3xl 2xl:text-4xl font-black text-slate-800 leading-none">{loadingEvaluacion ? <Skeleton className="h-6 xl:h-9 w-20 xl:w-32" /> : `${metricas.ent}/${metricas.inscritos}`}</h3>
                    </div>
